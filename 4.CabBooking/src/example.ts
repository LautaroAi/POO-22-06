...
async function main() {
  ...
  const driver1 = app.registerDriver('D1', 'Carlos', VehicleType.SEDAN, new GeoLocation(-33.45, -70.65));
  const driver2 = app.registerDriver('D2', 'María', VehicleType.PICKUP, new GeoLocation(-33.46, -70.66));
  const driver3 = app.registerDriver('D3', 'Juan', VehicleType.CUPE, new GeoLocation(-33.44, -70.64));

  // 2. Registrar pasajero
  const passenger = app.registerPassenger('P1', 'Ana', new GeoLocation(-33.45, -70.65));

  console.log('=== CONDUCTORES DISPONIBLES ===');
  const nearby = app.getNearbyDrivers(passenger.currentGeoLocation, 2);
  ...
  const trip = app.requestTrip(
    'P1',
    new GeoLocation(-33.45, -70.65),
    new GeoLocation(-33.50, -70.70),
